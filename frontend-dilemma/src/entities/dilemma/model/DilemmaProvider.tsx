...
import type { Choice, DilemmaType } from "@/shared/types";
import { fetchDilemmas, type ApiError } from "@/shared/lib/api";
import { loadFlowState, saveFlowState, clearFlowState } from "@/shared/lib/flowStateStorage";
import { useMyDecisions } from "@/entities/decision";
...
export function DilemmaProvider({ children }: { children: ReactNode }) {
  ...
  const { decisions: myDecisions, refresh: refreshMyDecisions } = useMyDecisions();

  const [currentDilemma, setCurrentDilemma] = useState<DilemmaType | null>(null);
