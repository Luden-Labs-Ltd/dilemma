...
## Ссылка на главную страницу (`VITE_APP_URL`)

На экране «Поделиться» (`/share`) QR‑код и ссылка ведут на главную страницу приложения. URL задаётся переменной окружения `VITE_APP_URL`:

- Если `VITE_APP_URL` **не задан** — используется текущий origin (например, `http://localhost:3000/` в dev или домен, на котором открыт сайт).
- Если задан (например, `VITE_APP_URL=https://dilemma-iota.vercel.app/`) — используется это значение.
